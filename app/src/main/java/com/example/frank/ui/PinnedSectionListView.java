...
import com.example.frank.test.BuildConfig;
...
public class PinnedSectionListView extends ExpandableListView {
    ...
    private boolean isAdapterReg = false;
    private BaseExpandableListAdapter mAdapter;
    //-- inner classes

	/** List adapter to be implemented for being used with PinnedSectionListView adapter. */
	public static interface PinnedSectionListAdapter extends  ExpandableListAdapter {
	...
	private final OnScrollListener mOnScrollListener = new OnScrollListener() {
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            BaseExpandableListAdapter adapter = getExpandableAdapter();
            if (adapter == null || visibleItemCount == 0) return; // nothing to do

            final boolean isFirstVisibleItemSection =
                    isItemViewTypePinned(adapter, adapter.getGroupType(firstVisibleItem));
	...
	void createPinnedShadow(int position) {
     ...
     //   BaseExpandableListAdapter adapter = getExpandableAdapter();
    //    int viewType = adapter.getGroupType(position);
     //   boolean isExpand = isItemViewTypePinned(adapter, viewType) ;

		View pinnedView = getExpandableAdapter().getGroupView(position, true,pinnedShadow.view, PinnedSectionListView.this);
		...
		pinnedShadow.id = getExpandableAdapter().getGroupId(position);
	...
	int findFirstVisibleSectionPosition(int firstVisibleItem, int visibleItemCount) {
        ...
        int adapterDataCount = adapter.getGroupCount();
	...
	int findCurrentSectionPosition(int fromPosition) {
		BaseExpandableListAdapter adapter = getExpandableAdapter();
		if (fromPosition >= adapter.getGroupCount()) return -1; // dataset has changed, no candidate
		...
		if (adapter instanceof SectionIndexer) {
			...
			int typeView = adapter.getGroupType(itemPosition);
...
//                        " of views in getViewTypeCount() method: items and sections?");
        ...
        BaseExpandableListAdapter oldAdapter =  getExpandableAdapter();
        if (oldAdapter != null && isAdapterReg) oldAdapter.unregisterDataSetObserver(mDataSetObserver);
        isAdapterReg  = true;
    ...
    private boolean performPinnedItemClick() {
        ...
        if (listener != null && getExpandableAdapter().isChildSelectable(mPinnedSection.position,0)) {
            View view =  mPinnedSection.view;
    ...
    public static boolean isItemViewTypePinned(BaseExpandableListAdapter adapter, int viewType) {
